...
export default function CroppedModal(props: CroppedModalProps) {
  const { listImage, visible, handleCancel, handleSave } = props;
  ...
  const [imageResults, dispatch] = React.useReducer<
    Reducer<ImageResult[], ImageAction>
  >(imageReducer, []);
          ...
          <div className="cropped-modal__content">
            ...
            <div className="cropped-modal__list">
              {listImage.map((currentImage, index) => {
                return (
                  <div
                    className="cropped-modal__image"
                    key={index}
                    onClick={() => {
                      handleSelect(index);
                    }}
